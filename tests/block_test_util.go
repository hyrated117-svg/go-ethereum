...
func (t *BlockTest) createTestBlockChain(config *params.ChainConfig, snapshotter bool, scheme string, witness bool, tracer *tracing.Hooks) (*core.BlockChain, error) {
...
func (t *BlockTest) Run(snapshotter bool, scheme string, witness, createAndVerifyBAL bool, tracer *tracing.Hooks, postCheck func(error, *core.BlockChain)) (result error) {
	...
	chain, err := t.createTestBlockChain(config, snapshotter, scheme, witness, tracer)
	if err != nil {
		return err
	}
	defer chain.Stop()
	...
	if createAndVerifyBAL {
		newChain, err := t.createTestBlockChain(config, snapshotter, scheme, witness, tracer)
		if err != nil {
			return err
		}
		defer newChain.Stop()
		...
		if _, err := newChain.InsertChain(blocksWithBAL); err != nil {
			return err
		}
		...
		if err := t.validateImportedHeaders(newChain, validBlocks); err != nil {
			return err
		}
	}
	return nil
